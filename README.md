...
- **Name** and **working directory**, with a small **project badge** in front of
  the name — the repo the session is working in (found by walking up to the `.git`
  root; for a git **worktree** it shows the *main* project, not the worktree
  folder). The working-directory line stays muted but highlights the worktree
  folder, so you can still tell which worktree at a glance.
